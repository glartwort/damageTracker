...
Hooks.on("createChatMessage", (message, data, options, userId) => {
  ...
  if (message.flags.pf2e.context?.type === "damage-taken") {
    ProcessDamageTaken(message,data)
  }
  ...
  if (message.flags.pf2e.context?.type === "damage-roll") {
    ProcessDamageRoll(message, data);
  }
});
...
Hooks.on("updateChatMessage", (message, data, options, userId) => {
  ...
  if (message.flags.pf2e.context?.type === "damage-taken") {
    ...
    {
      ProcessDamageRevert(message, data);
    }
  }
});

function ProcessDamageTaken(message,data) {
  ...
  const targetId =      message.flags.pf2e?.appliedDamage?.uuid;
  const targetActor =   (targetId)?fromUuidSync(targetId):null;
  let target = UNIDENTIFIED_ATTACKER;
  if (targetActor.token)
    target = targetActor.token.name;    //use token name if it's available
  else
    target = targetActor.name;          //fall back to actor name if it's not.

  const targetIsNPC =  (targetActor?.type !== "character");   
...
function ProcessDamageRoll(message, data) {
  const isNPCLoggingEnabled =  game.settings.get(MODULE_ID, "enableNPCTracking");
  ...
  if ((!isNPC) || (isNPCLoggingEnabled)) {
    ...
    if (isDebug) {
      console.log(LOG_PREFIX, "Damage Roll!");
      console.log(LOG_PREFIX, "damage Amount: \t", damageRoll);
...
function ProcessDamageRevert(message,data) {
  ...
  RevertDamage(attackerId, damageRoll, damage);
   ...
   if ((!targetIsNPC) || (isNPCLoggingEnabled)) {
      RevertMostDmgTaken(targetId, damage);
    }
}
...
async function StashDamageRoll(key, name, isNPC, damageRoll) {
  ...
  if (!actorMap[key]) {   //create new actor in damageRolls  (this is likely to be where all new actors get created.)
    ...
    if (isDebug) console.log(LOG_PREFIX, "Merged data into existing", (isNPC)?"NPC":"PC", "for:", name, "with", damageRoll, "damage.");
  }
...
// Return true if something changed, otherwise false.
function checkAndUpdateMostDmgTaken(actor,damage) {
  let currentMax = (actor.mostDmgTaken)?actor.mostDmgTaken:0;
  
  if (damage > currentMax) {
    actor.prevMostDmgTaken = currentMax;
    actor.mostDmgTaken = damage;
    return true;
  } 
  // if the roll isn't bigger than the current max, but it is bigger than previous, update previous max.
  // Explicity not doing anything on damageRoll == currentMax since this potentially gets called twice
  // for damaging someone (damage roll, then damage hit).. If I recorded both it wouldn't revert (max and previous would be the same).
  if ((damage > actor.prevMostDmgTaken) && (damage < currentMax)) {
    actor.prevMostDmgTaken = damage;
    return true;
  }
  return false;
}
...
async function AddorMergeActorMostDmgTaken(key, name, isNPC, damage) {
  const actorMap = game.settings.get(MODULE_ID, "damageMap") ?? {};
  ...
  if (!actorMap[key]) {   
    ...
    actorMap[key].name = name;
    actorMap[key].isNPC = isNPC;
    actorMap[key].mostDmgTaken = damage;
    actorMap[key].prevMostDmgTaken = 0;
    
    if (isDebug) console.log(LOG_PREFIX, "Created new", (isNPC)?"NPC":"PC", "for:", name, "has taken", damage, "damage.");
  } else {    //actor already exists, update
    const existing = actorMap[key];
    if (!checkAndUpdateMostDmgTaken(existing, damage)) {
      // short circuit, no changes are required, don't bother setting data
      return;
    }
      
    if (isDebug) console.log(LOG_PREFIX, "Merged data into existing", (isNPC)?"NPC":"PC", "for:", name, "has taken", damage, "damage.");
  } 
  await game.settings.set(MODULE_ID, "damageMap", actorMap);
}
...
async function RevertMostDmgTaken(key, damage) {
  const actorMap = game.settings.get(MODULE_ID, "damageMap") ?? {};
    
  if (!actorMap[key]) {  
    if (isDebug) console.log(LOG_PREFIX, "Actor is not in the table to revert most damage taken from");
  } else {    //actor already exists, update
    const actor = actorMap[key];
    const Actortype = (actor.isNPC)?"NPC":"PC";
    
    if (actor.mostDmgTaken == damage) {
      actor.mostDmgTaken = actor.prevMostDmgTaken;
      actor.prevMaxDmgRoll = 0;     //don't save the current or it could never revert that roll.  Shouldn't happen, but just in case.
      if (isDebug) console.log(LOG_PREFIX, "Reverted damage from existing", (actor.isNPC)?"NPC":"PC", "for:", actor.name, "with", damage, "damage.");
